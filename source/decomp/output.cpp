...
template <class Container, class F> void writeJsonArray(std::ostream& out, Container const& container, F f)
{
    out << "[";
...
void svg::writePolygon(std::ostream& svg,
...
std::ostream& json::dump(std::ostream& out,
                         ...
                         std::vector<IndexList> const& convexPolygonList)
{
    ...
    writeJsonArray(out, pointList, [](std::ostream& os, Point const& p) { os << "[" << p[0] << "," << p[1] << "]"; });
    out << "," << std::endl;
    out << "\"input\": {" << std::endl;
    out << "\"outer\": " << std::endl;
    writeJsonArray(out, outerPolygon, [](std::ostream& os, std::uint16_t const& p) { os << p; });
    out << "," << std::endl;
    out << "\"holes\": " << std::endl;
